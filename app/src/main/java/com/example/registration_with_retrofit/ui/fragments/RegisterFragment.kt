...
import com.example.registration_with_retrofit.databinding.FragmentRegisterBinding
...
import com.example.registration_with_retrofit.viewmodel.RegisterViewModel
        ...
        binding.btnRegister.setOnClickListener {
            if (!binding.root.areLinesEmpty() && requireContext().isOnline()) {
                viewLifecycleOwner.lifecycleScope.launch {
                    viewModel.registerUser(getUser()).collect {
                        handleResponse(it)
                    }
                }
            }
